...
describe("Marketplace", function () {
  ...
  describe("claim", function () {
    ...
    let endTime;
    let createdOn;
    it("Should create claimable auction listing for bidder", async function () {
      let nftId = 0;
      let toEOA = true;
      let depositType = 1;
      ...
      startTime = ethers.BigNumber.from((await time.latest()).toString());
      endTime = startTime.add(time.duration.days(2).toString());
    ...
    it("Should validate all claimable conditions", async function () {
      let nftId = 0;
      let toEOA = true;
      let depositType = 1;
      let activeDepositType = 1;
      ...
      await expect(
        swaprGL.isClaimable(bidder.address, listingData, proposedBidData)
      ).to.be.revertedWith("INV_DATA");
      await expect(
        swaprGL.isClaimable(owner.address, listingData, proposedBidData)
      ).to.be.revertedWith("INV_DATA");
      ...
      expect(
        (
          await swaprGL.isClaimable(owner.address, listingData, proposedBidData)
        )[0]
      ).to.be.false;
      ...
      const resp = await swaprGL.isClaimable(
        account2.address,
        listingData,
        proposedBidData
      );
      expect(resp[0]).to.be.equal(false);

      const resp1 = await swaprGL.isClaimable(
        bidder.address,
        listingData,
        proposedBidData
      );
      expect(resp1[0]).to.be.equal(true);
      const resp2 = await swaprGL.isClaimable(
        owner.address,
        listingData,
        proposedBidData
      );
      expect(resp2[0]).to.be.equal(true);
    });

    it("Should claim as bidder", async function () {
      let nftId = 0;
      let toEOA = true;
      let depositType = 1;
      let activeDepositType = 1;
      ...
      const txs = await swaprGL.connect(bidder).claim(data);
      const rc = await txs.wait();
      const event = rc.events.find((event) => event.event === "Claimed");
      const [success, res] = event.args;
      if (!success) {
        console.log("FAIL REASON: ", res);
      }
    ...
    it("Should create claimable auction listing for seller", async function () {
      let newIDs;
      if (
    ...
    it("Should claim as seller", async function () {
      let nftId = 2;
      let toEOA = true;
      let depositType = 1;
      let activeDepositType = 1;
      ...
      const txs = await swaprGL.connect(owner).claim(data);
      const rc = await txs.wait();
      const event = rc.events.find((event) => event.event === "Claimed");
      const [success, res] = event.args;
      if (!success) {
